...
export const getBranchProducts = async (
  ...
  try {
    ...
    console.error(
      `Error obteniendo productos de ${branch}:`,
      error
    );
    return [];
  }
...
export const getBeneficiarios = async () => {
  try {
    const response = await api.get(
      `/api/${CURRENT_NODE}/beneficiarios`
    );
    return response.data;
  } catch (error) {
    console.error(
      "Error obteniendo beneficiarios:",
      error
    );
    return [];
  }
...
export const getCategories = async () => {
  const response =
    await axios.get(
      `${API_URL}/api/${CURRENT_NODE}/categorias`
    );
  return response.data;
};
...
export const requestProduct =
  async (
    ...
    try {
      ...
      console.error(
        `Error solicitando producto a ${sourceBranchKey}:`,
        error
      );
      throw error;
    }
...
export const createSolicitud =
  async ({
    ...
    try {
      const response =
        await api.post(
          `/api/${CURRENT_NODE}/transferencias`,
          {
            producto_nombre,
            cantidad,
            origen,
...
export const getSolicitudesEnviadas =
  async () => {
    try {
      const response =
        await api.get(
      ...
      return todas
        .filter(
          (t) =>
            t.origen?.toLowerCase() ===
            CURRENT_NODE.toLowerCase()
        )
        .sort(
          (a, b) =>
            new Date(
...
export const getSolicitudesRecibidas =
  async () => {
    try {
      const respuestas =
        await Promise.all(
